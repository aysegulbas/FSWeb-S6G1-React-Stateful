...
export default function Programcilar() {
    ...
    const [onecıkanid,setonecıkanid]=useState(null)
...
let isimbul=enIyilerListesi.find(kisi=>kisi.id===onecıkanid)
return isimbul.isim

  //  if(onecıkanid==enIyilerListesi[i].id)
  ...
  const stil = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: onecıkanid===null?'royalblue':"gold", // 🤔 kutlarken renk gold'a dönecek
  };
      ...
      <div className='programmers'>
        {
          ...
          eniyiler.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => {setonecıkanid(dev.id) }}>Kutla</button>
            </div>
          )
        }
      </div>
